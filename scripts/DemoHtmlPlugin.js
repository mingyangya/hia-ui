...
/**
 * Webpack 插件：生成 demo.html 文件
 */
class DemoHtmlPlugin {
  constructor(options = {}) {
    this.options = {
      filename: 'demo.html',
      title: 'HiaUi Demo',
  ...
  apply(compiler) {
    // 监听构建完成事件
    compiler.hooks.afterEmit.tapAsync('DemoHtmlPlugin', (compilation, callback) => {
      // 只有在 lib 构建时生成 demo.html
      const isLib = process.argv.includes('--target') && process.argv.includes('lib');
      if (!isLib) {
        return callback();
      }

      const demoContent = this.generateDemoHtml();
      const outputPath = compilation.options.output.path;
      const demoPath = path.join(outputPath, this.options.filename);

      // 写入 demo.html 文件
      fs.writeFile(demoPath, demoContent, (err) => {
        if (err) {
          console.error('❌ demo.html 生成失败:', err);
        } else {
